// exaonemoe_engine.cpp — ExaoneMoe decoder (GQA + per-head q/k RMSNorm +
// rope + dense/MoE with group-limited sigmoid router + shared experts), CPU.
// Mirrors transformers modeling_exaone_moe.py 5.14 EXACTLY — validated
// against the numpy port (Testing/e2e_numpy_ref_exaonemoe.py).
//
// Block: input_layernorm -> GQA attention (q/k RMSNorm per head ALWAYS,
// rope (sliding layers rope; full_attention layers NoPE when sliding_window
// set), scaling head_dim^-0.5, repeat_interleave kv mapping) -> residual ->
// post_attention_layernorm -> [dense gelu gate/up/down MLP | MoE: sigmoid
// router + e_score_correction_bias, group-limited (top2 per group -> topk
// groups), topk, norm_topk_prob /sum, routed_scaling_factor, per-expert
// gate/up/down + shared_experts] -> residual. Final model.norm; UNTIED lm_head.
...
class ExaoneMoeBackend : public Backend {
public:
    ExaoneMoeBackend() { type = BackendType::GENERIC; name = "exaonemoe_cpu"; }
    ...
    bool init(const ModelConfig& cfg_, const std::string& dir) override {
        ...
        if (!ok) { fprintf(stderr, "[exmoe] open failed\n"); return false; }
        w_ = std::move(rdr);
...
private:
    ...
    int FF = 0, E = 0, TOPK = 0, EFF = 0, NSHARED = 1;
    int NG = 1, TG = 1;
    int seq_len = 0;
    float eps = 1e-5f, rope_theta = 10000.0f, routed_scale = 1.0f;
    bool norm_topk = true, has_sliding_window = false;
    size_t embed_w = SIZE_MAX, head_w = SIZE_MAX, final_norm = SIZE_MAX;
    std::vector<ExMoeLayer> layers;
    ...
    size_t store_t(const std::string& n, int rows, int cols = 1) {
        ...
        if (!w_.get_tensor_f32(n, v) || (int)v.size() != rows * cols) {
            fprintf(stderr, "[exmoe] missing/misized %s (%zu want %d)\n", n.c_str(), v.size(), rows * cols);
            return SIZE_MAX;
        }
    ...
    bool load_config(const std::string& dir) {
        ...
        { size_t p = txt.find("\"sliding_window\""); has_sliding_window = txt.find(':', p) != std::string::npos && txt.find_first_of("0123456789", p + 1) != std::string::npos; }
    ...
    bool load_weights() {
        ...
        for (int l = 0; l < L; l++) {
            ...
            if (!mlp_sparse[l]) {
                ...
                ly.expert_bias = store_t(mp + "e_score_correction_bias", E);
                ly.e_ff = EFF;
    ...
    void attention_mix(const float* x, const ExMoeLayer& ly, int l, int pos, float* out) {
        ...
        // rope on sliding layers (all layers here are sliding; full layers NoPE when sliding_window set)
    ...
    void mlp_mix(const float* x, const ExMoeLayer& ly, int sff, float* out) {
        std::vector<float> g(sff), u(sff);
        mm(ly.gate_w, x, H, sff, g.data());
        mm(ly.up_w, x, H, sff, u.data());
        std::vector<float> h(sff);
        for (int i = 0; i < sff; i++) h[i] = gelu(g[i]) * u[i];
        mm(ly.down_w, h.data(), sff, H, out);
    }

    void moe_mix(const float* x, const ExMoeLayer& ly, float* out) {
        std::vector<float> logits(E);
        mm(ly.router, x, H, E, logits.data());
        std::vector<float> scores(E), sc(E);
        const float* eb = wt(ly.expert_bias);
        for (int i = 0; i < E; i++) {
            scores[i] = 1.0f / (1.0f + std::exp(-logits[i]));
            sc[i] = scores[i] + eb[i];
        }
        // group-limited: top2 per group -> topk_group groups
        std::vector<float> group_scores(NG);
        for (int g = 0; g < NG; g++) {
            float m1 = -1e30f, m2 = -1e30f;
            for (int e = 0; e < E / NG; e++) {
                float s = sc[g * (E / NG) + e];
        ...
        bool gused[64] = {false};
        for (int rank = 0; rank < TG && rank < 64; rank++) {
            int best = -1; float bv = -1e30f;
            for (int g = NG - 1; g >= 0; g--) {
                if (gused[g]) continue;
                if (group_scores[g] > bv) { bv = group_scores[g]; best = g; }
            }
            gused[best] = true;
        }
        std::vector<float> sc_masked(E);
        for (int i = 0; i < E; i++)
            sc_masked[i] = gused[i / (E / NG)] ? sc[i] : -1e30f;
        int idx[64];
        ...
        for (int rank = 0; rank < TOPK && rank < 64; rank++) {
            ...
            for (int i = E - 1; i >= 0; i--) {
                if (used[i]) continue;
                if (sc_masked[i] > bv) { bv = sc_masked[i]; best = i; }
            }
            idx[rank] = best; used[best] = true;
        }
        float sum = 0;
        std::vector<float> rw(TOPK);
        for (int i = 0; i < TOPK && i < 64; i++) { rw[i] = scores[idx[i]]; sum += rw[i]; }
        std::vector<float> acc(H, 0.0f);
        for (int rank = 0; rank < TOPK && rank < 64; rank++) {
            float wtv = norm_topk ? rw[rank] / sum : rw[rank];
            wtv *= routed_scale;
            int e = idx[rank];
            const float* w1 = weights_.data() + ly.experts_base + (size_t)e * 3 * EFF * H;  // gate
...
extern "C" Backend* create_exaonemoe_backend() { return new ExaoneMoeBackend(); }
