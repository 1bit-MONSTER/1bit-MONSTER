...
static void upf16(const std::vector<float>& s,__half*d,int n,hipStream_t h=0){
    ...
    // Chunk the copy: hipMemcpyAsync takes a signed 32-bit byte count, so a
    // single tensor over 2 GiB (e.g. the 74B's embed, 262147x4096x2B) returns
    // hipErrorInvalidValue. Split into 1 GiB pieces (issue #1715).
    const size_t total = (size_t)n * 2;
    const size_t chunk = (size_t)1 << 30;  // 1 GiB
    for (size_t off = 0; off < total; off += chunk) {
        size_t bytes = total - off < chunk ? total - off : chunk;
        HIP_OK_V(hipMemcpyAsync((char*)d + off, (const char*)b.data() + off, bytes, hipMemcpyHostToDevice, h));
    }
}
static void upf32(const std::vector<float>& s,float*d,int n,hipStream_t h=0){
    ...
    // Chunked the same way as upf16: sizes above 2 GiB overflow the signed
    // 32-bit hipMemcpyAsync byte count (issue #1715).
    const size_t total = (size_t)n * 4;
    const size_t chunk = (size_t)1 << 30;  // 1 GiB
    for (size_t off = 0; off < total; off += chunk) {
        size_t bytes = total - off < chunk ? total - off : chunk;
        HIP_OK_V(hipMemcpyAsync((char*)d + off, (const char*)s.data() + off, bytes, hipMemcpyHostToDevice, h));
    }
}
...
void zaya_forward(ZayaState* s, int token_id, float* logits_out) {
    ...
    for(int il=0;il<eng.n_layers;il++){
        ...
        if((l.gu||l.gu_q)&&(l.dn||l.dn_q)){
            ...
            // Router: score ALL n_exp_t slots (softmax over N, top-1), matching
            // the upstream llama.cpp Zaya graph (PR #23112 — issue #1713 oracle
            // check). The old router_top_k=2 scoring could never select experts
            // 2..n_exp-1; slot n_exp is the skip expert, folded to expert 0 +
            // skip_flag by fixup_skip_expert_kernel below.
...
void zaya_forward_batch(ZayaState* s, const int* token_ids, float* logits_out, int B) {
    ...
    // The batch path is validated for fp16 weights only: its attention and
    // MoE kernels take fp16 pointers (l.wv1/l.wo/l.gu/l.dn), which are NULL
    // for Q4NX-packed models (uploaded via the *_q pointers by the onebp
    // loader — issue #1723). Defer to the per-token reference for Q4NX
    // instead of dereferencing NULL (the batch path is a test-only
    // single-position evaluator, issue #1714).
    {
        bool has_q4nx = false;
        for (int il = 0; il < eng.n_layers && !has_q4nx; il++) {
            const auto& l = s->lw[il];
            has_q4nx = l.wq_q || l.wk_q || l.wv1_q || l.wv2_q || l.wo_q || l.gu_q || l.dn_q;
        }
        if (has_q4nx) {
            if (logits_out)
                for (int b = 0; b < B; b++)
