// ernie45moe_engine.cpp — Ernie4.5-MoE decoder (GQA + RoPE + softmax-router
// MoE with e_score_correction_bias + shared experts), CPU. Mirrors
// transformers modeling_ernie4_5_moe.py 5.14 EXACTLY — validated against the
// numpy port (Testing/e2e_numpy_ref_ernie45moe.py).
//
// Block: input_layernorm -> GQA attention (full rope, scaling=head_dim^-0.5,
// repeat_interleave kv mapping) -> residual -> post_attention_layernorm ->
// [dense gelu gate/up/down MLP | MoE for layers where
// (l+1)%moe_layer_interval==0 && l in [start,end]: softmax router +
// e_score_correction_bias (topk on adjusted scores, gather RAW softmax
// weights, /clamp(sum, min=moe_norm_min)), per-expert gate/up/down +
// shared_experts] -> residual. Final model.norm; UNTIED lm_head.
// NOTE: torch.topk tie-break picks the HIGHER index on equal scores —
// matters for degenerate (zero) routers; replicated exactly.
...
class Ernie45MoeBackend : public Backend {
public:
    Ernie45MoeBackend() { type = BackendType::GENERIC; name = "ernie45moe_cpu"; }
    ...
    bool init(const ModelConfig& cfg_, const std::string& dir) override {
        ...
        if (!ok) { fprintf(stderr, "[e45] open failed\n"); return false; }
        w_ = std::move(rdr);
...
private:
    ...
    int FF = 0, E = 0, TOPK = 0, EFF = 0, NSHARED = 0;
    int moe_start = 0, moe_end = 0, moe_interval = 1;
    int seq_len = 0;
    float eps = 1e-5f, rope_theta = 10000.0f, norm_min = 1e-12f;
    size_t embed_w = SIZE_MAX, head_w = SIZE_MAX, final_norm = SIZE_MAX;
    std::vector<ErnieMoeLayer> layers;
    ...
    size_t store_t(const std::string& n, int rows, int cols = 1) {
        ...
        if (!w_.get_tensor_f32(n, v) || (int)v.size() != rows * cols) {
            fprintf(stderr, "[e45] missing/misized %s (%zu want %d)\n", n.c_str(), v.size(), rows * cols);
            return SIZE_MAX;
        }
    ...
    bool load_config(const std::string& dir) {
        ...
        find_int("moe_num_experts", E);
        find_int("moe_k", TOPK);
        find_int("moe_num_shared_experts", NSHARED);
        find_int("moe_layer_start_index", moe_start);
        find_int("moe_layer_end_index", moe_end);
        find_int("moe_layer_interval", moe_interval);
        find_float("rms_norm_eps", eps);
        find_float("moe_norm_min", norm_min);
    ...
    bool load_weights() {
        ...
        for (int l = 0; l < L; l++) {
            ...
            bool is_moe = ((l + 1) % moe_interval == 0) && l >= moe_start && l <= moe_end;
            std::string mp = pfx + "mlp.";
            if (is_moe) {
                ly.is_moe = 1;
                ly.gate_w = store_t(mp + "gate.weight", E, H);
                ly.expert_bias = store_t(mp + "moe_statics.e_score_correction_bias", E);
                ly.e_ff = EFF;
                ...
                // dense MLP: gate_w=gate, expert_bias=up, experts_base=down
                ly.ff = FF;
                ly.gate_w = store_t(mp + "gate_proj.weight", FF, H);
                ly.expert_bias = store_t(mp + "up_proj.weight", FF, H);
                ly.experts_base = (int)store_t(mp + "down_proj.weight", H, FF);
            }
        }
        return true;
    }
    ...
    void mlp_mix(const float* x, const ErnieMoeLayer& ly, float* out) {
        std::vector<float> g(ly.ff), u(ly.ff);
        mm(ly.gate_w, x, H, ly.ff, g.data());
        mm(ly.expert_bias, x, H, ly.ff, u.data());  // up_proj stored in expert_bias slot for dense
        std::vector<float> h(ly.ff);
        for (int i = 0; i < ly.ff; i++) h[i] = gelu(g[i]) * u[i];
        mm((size_t)ly.experts_base, h.data(), ly.ff, H, out);  // down_proj
    }

    void moe_mix(const float* x, const ErnieMoeLayer& ly, float* out) {
        std::vector<float> logits(E);
        mm(ly.gate_w, x, H, E, logits.data());
        // softmax
        float mx = logits[0];
        for (int i = 1; i < E; i++) if (logits[i] > mx) mx = logits[i];
        std::vector<float> routing(E), scores(E);
        float sum_all = 0;
        for (int i = 0; i < E; i++) { routing[i] = std::exp(logits[i] - mx); sum_all += routing[i]; }
        for (int i = 0; i < E; i++) routing[i] /= sum_all;
        const float* eb = wt(ly.expert_bias);
        for (int i = 0; i < E; i++) scores[i] = routing[i] + eb[i];
        if (seq_len == 6) { fprintf(stderr, "[dbg] l%d logits: %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n", (int)(&ly - layers.data()), logits[0],logits[1],logits[2],logits[3],logits[4],logits[5],logits[6],logits[7]); }
        // topk with torch tie-break (higher index wins on equal scores)
        int idx[64];
        bool used[1024] = {false};
        for (int rank = 0; rank < TOPK && rank < 64; rank++) {
            int best = -1; float bv = -1e30f;
            for (int i = E - 1; i >= 0; i--) {  // scan high->low; strict > keeps
                if (used[i]) continue;          // the FIRST (highest-index) max on ties
                if (scores[i] > bv) { bv = scores[i]; best = i; }
            }
            idx[rank] = best; used[best] = true;
            if (seq_len == 6) fprintf(stderr, "[dbg] rank %d picks expert %d score %.6f\n", rank, best, bv);
        }
        float sum = 0;
        for (int i = 0; i < TOPK && i < 64; i++) sum += routing[idx[i]];
        if (sum < norm_min) sum = norm_min;
        std::vector<float> acc(H, 0.0f);
        for (int i = 0; i < TOPK && i < 64; i++) {
            float wtv = routing[idx[i]] / sum;
        ...
        if (ly.sff > 0) {
            ...
            for (int i = 0; i < ly.sff; i++) sh[i] = gelu(sg[i]) * su[i];
...
extern "C" Backend* create_ernie45moe_backend() { return new Ernie45MoeBackend(); }
