// hyv3_engine.cpp — HY-V3 decoder (GQA attention + q/k RMSNorm + RoPE,
// dense-then-MoE with sigmoid router + expert bias + shared experts), CPU.
// Mirrors transformers modeling_hy_v3.py 5.14 EXACTLY — validated against the
// numpy port (Testing/e2e_numpy_ref_hyv3.py, corr 1.0000 / top-5 exact on
// yujiepan/hy-v3-tiny-random).
//
// Block: input_layernorm -> GQA attention (q/k RMSNorm per head, full rope,
// scaling=head_dim^-0.5, repeat_interleave kv mapping) -> residual ->
// post_attention_layernorm -> [dense gate/up/down silu MLP | MoE: sigmoid
// router + e_score_correction_bias selection, top-k, /(sum+1e-20),
// router_scaling_factor, per-expert gate/down/up + shared_mlp] -> residual.
// Final model.norm; UNTIED lm_head (tie_word_embeddings=false).
// nextn (MTP) predictor layer excluded exactly like transformers
...
struct Hyv3Layer {
    ...
    size_t gate_proj = SIZE_MAX, up_proj = SIZE_MAX, down_proj = SIZE_MAX;  // dense
...
class Hyv3Backend : public Backend {
public:
    Hyv3Backend() { type = BackendType::GENERIC; name = "hyv3_cpu"; }
    ...
    bool init(const ModelConfig& cfg_, const std::string& dir) override {
        ...
        if (!ok) { fprintf(stderr, "[hyv3] open failed\n"); return false; }
        w_ = std::move(rdr);
...
private:
    ...
    int seq_len = 0;
    float eps = 1e-5f, rope_theta = 10000.0f, router_scale = 1.0f;
    size_t embed_w = SIZE_MAX, head_w = SIZE_MAX, final_norm = SIZE_MAX;
    std::vector<Hyv3Layer> layers;
    std::vector<int> layer_type;  // 0 = dense, 1 = moe
    ...
    size_t store_t(const std::string& n, int rows, int cols = 1) {
        ...
        if (!w_.get_tensor_f32(n, v) || (int)v.size() != rows * cols) {
            fprintf(stderr, "[hyv3] missing/misized %s (%zu want %d)\n", n.c_str(), v.size(), rows * cols);
            return SIZE_MAX;
        }
    ...
    bool load_config(const std::string& dir) {
        ...
        find_float("router_scaling_factor", router_scale);
    ...
    bool load_weights() {
        ...
        for (int l = 0; l < L; l++) {
            ...
            if (layer_type[l] == 0) {
                ly.gate_proj = store_t(mp + "gate_proj.weight", FF, H);
                ly.up_proj = store_t(mp + "up_proj.weight", FF, H);
                ly.down_proj = store_t(mp + "down_proj.weight", H, FF);
                ly.ff = FF;
            } else {
                ly.is_moe = 1;
                ...
                int sff = EFF * NSHARED;
                ly.shared_gate = store_t(mp + "shared_mlp.gate_proj.weight", sff, H);
                ly.shared_up = store_t(mp + "shared_mlp.up_proj.weight", sff, H);
                ly.shared_down = store_t(mp + "shared_mlp.down_proj.weight", H, sff);
            }
        }
        return true;
    }
    ...
    void mlp_mix(const float* x, const Hyv3Layer& ly, int sff, float* out) {
        std::vector<float> g(sff), u(sff);
        mm(ly.gate_proj, x, H, sff, g.data());
        mm(ly.up_proj, x, H, sff, u.data());
        std::vector<float> h(sff);
        for (int i = 0; i < sff; i++) h[i] = silu(g[i]) * u[i];
        mm(ly.down_proj, h.data(), sff, H, out);
    }

    void moe_mix(const float* x, const Hyv3Layer& ly, float* out) {
        ...
        for (int i = 0; i < TOPK && i < 64; i++) {
            float wtv = rw[i] * router_scale / sum;
        ...
        // shared experts
        int sff = EFF * NSHARED;
        std::vector<float> sg(sff), su(sff), sh(sff), so(H);
        mm(ly.shared_gate, x, H, sff, sg.data());
        mm(ly.shared_up, x, H, sff, su.data());
        for (int i = 0; i < sff; i++) sh[i] = silu(sg[i]) * su[i];
        mm(ly.shared_down, sh.data(), sff, H, so.data());
...
extern "C" Backend* create_hyv3_backend() { return new Hyv3Backend(); }
