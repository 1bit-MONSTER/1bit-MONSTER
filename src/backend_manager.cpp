...
void BackendManager::discover() {
    ...
    // 2c5. MiniMax-M2 CPU — GQA + flattened q/k RMSNorm + partial rope + sigmoid MoE.
    {
        BackendInfo info;
        info.id = "minimaxm2_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "MiniMax-M2 (GQA + sigmoid MoE) CPU";
        info.priority = tier_priority(info.tier) + 32;
        info.available = true;
        info.functional = false;
        info.score = 0;
        info.total_inferences = 0;
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "MiniMax-M2 CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }

    // 2c6. Falcon-H1 CPU — Mamba-2 SSM + GQA + MuP multipliers hybrid.
    {
        BackendInfo info;
        info.id = "falconh1_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "Falcon-H1 (Mamba2+attn+MuP) CPU";
        info.priority = tier_priority(info.tier) + 33;
        info.available = true;
        info.functional = false;
        info.score = 0;
        info.total_inferences = 0;
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "Falcon-H1 CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }
    ...
    // 2cb. AfMoE CPU — dual-norm GQA + sigmoid-gated sliding attn + shared-expert MoE.
    {
        BackendInfo info;
        info.id = "afmoe_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "AfMoE (dual-norm GQA+MoE) CPU";
        info.priority = tier_priority(info.tier) + 38;
        info.available = true;
        info.functional = false;
        info.score = 0;
        info.total_inferences = 0;
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "AfMoE CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }
    ...
    // 2ce. PhiMoE CPU — GQA + LayerNorm + sparsemixer MoE.
    {
        BackendInfo info;
        info.id = "phimoe_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "PhiMoE (GQA+LayerNorm+sparsemixer-MoE) CPU";
        info.priority = tier_priority(info.tier) + 41;
        info.available = true;
        info.functional = false;
        info.score = 0;
        info.total_inferences = 0;
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "PhiMoE CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }
    ...
    // 2ch. ExaoneMoe CPU — GQA + q/k RMSNorm + group-limited MoE + shared experts.
    {
        BackendInfo info;
        info.id = "exaonemoe_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "ExaoneMoe (GQA+q/k-RMSNorm+group-MoE) CPU";
        info.priority = tier_priority(info.tier) + 44;
        info.available = true;
        info.functional = false;
        info.score = 0;
        info.total_inferences = 0;
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "ExaoneMoe CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }

    // 2ci. FalconMamba CPU — Mamba1 SSM + RMSNorm on B/C/dt.
    {
        BackendInfo info;
        info.id = "falconmamba_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "FalconMamba (Mamba1 SSM) CPU";
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "FalconMamba CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }

    // 2cj. JetMoE CPU — Mixture of Attention + MoE FFN.
    {
        BackendInfo info;
        info.id = "jetmoe_cpu";
        info.type = BackendType::GENERIC;
        info.tier = BackendTier::T3_CPU;
        info.description = "JetMoE (MoA+MoE) CPU";
        info.priority = tier_priority(info.tier) + 46;
        info.available = true;
        info.functional = false;
        info.score = 0;
        info.total_inferences = 0;
        ...
        info.instance = nullptr;
        info.plugin_handle = nullptr;
        printf("  %-25s %s\n", "JetMoE CPU", info.available ? "✅ detected" : "❌ not available");
        backends_.push_back(info);
    }
...
Backend* BackendManager::create_instance_rt(const BackendInfo& info) {
    ...
    switch (info.type) {
        case BackendType::HIP_GPU:
            ...
            // Falcon-H1 backend (Mamba-2 SSM + GQA + MuP multipliers)
            if (info.id == "falconh1_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_falconh1_backend");
            ...
            // AfMoE backend (dual-norm GQA + sigmoid-gated sliding attn + MoE)
            if (info.id == "afmoe_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_afmoe_backend");
            ...
            // PhiMoE backend (GQA + LayerNorm + sparsemixer MoE)
            if (info.id == "phimoe_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_phimoe_backend");
            ...
            // ExaoneMoe backend (GQA + q/k RMSNorm + group-limited MoE + shared experts)
            if (info.id == "exaonemoe_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_exaonemoe_backend");
                if (!b) b = try_load_backend("libexaonemoe_backend.so", "create_exaonemoe_backend");
                if (!b) { void* self = dlopen(NULL, RTLD_NOW|RTLD_LOCAL);
                    if (self) { auto* fn = (Backend*(*)())dlsym(self, "create_exaonemoe_backend");
                        if (fn) b = fn(); } }
                return b;
            }
            // FalconMamba backend (Mamba1 SSM + RMSNorm on B/C/dt)
            if (info.id == "falconmamba_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_falconmamba_backend");
                if (!b) b = try_load_backend("libfalconmamba_backend.so", "create_falconmamba_backend");
                if (!b) { void* self = dlopen(NULL, RTLD_NOW|RTLD_LOCAL);
                    if (self) { auto* fn = (Backend*(*)())dlsym(self, "create_falconmamba_backend");
                        if (fn) b = fn(); } }
                return b;
            }
            // JetMoE backend (Mixture of Attention + MoE FFN)
            if (info.id == "jetmoe_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_jetmoe_backend");
                if (!b) b = try_load_backend("libjetmoe_backend.so", "create_jetmoe_backend");
                if (!b) { void* self = dlopen(NULL, RTLD_NOW|RTLD_LOCAL);
                    if (self) { auto* fn = (Backend*(*)())dlsym(self, "create_jetmoe_backend");
                        if (fn) b = fn(); } }
                return b;
            }
            // MiniMax-M2 backend (GQA + flattened q/k RMSNorm + sigmoid MoE)
            if (info.id == "minimaxm2_cpu") {
                b = try_load_backend("librocm_cpp.so", "create_minimaxm2_backend");
                if (!b) b = try_load_backend("libminimaxm2_backend.so", "create_minimaxm2_backend");
                if (!b) { void* self = dlopen(NULL, RTLD_NOW|RTLD_LOCAL);
                    if (self) { auto* fn = (Backend*(*)())dlsym(self, "create_minimaxm2_backend");
                        if (fn) b = fn(); } }
                return b;
            }
