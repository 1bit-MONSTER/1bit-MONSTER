// cmp_mimo_v2.cpp — MiMo-V2-Flash engine e2e gate.
//
// usage: cmp_mimo_v2 <model_dir> <ids.txt> <hf_logits.npy> [topN] [min_overlap]
#include <cstdio>
#include <fstream>
#include <sstream>
#include <string>
...
int main(int argc, char** argv) {
    if (argc < 4) { printf("usage: cmp_mimo_v2 <model_dir> <ids.txt> <hf_logits.npy> [topN] [min_overlap]\n"); return 2; }
    ...
    MiMoV2KVCache kv_cache;
    int pos = 0;
    std::vector<float> last_logits;
    for (size_t i = 0; i < ids.size(); i++) {
        last_logits = mimo_v2_forward(model, ids[i], kv_cache, pos);
    }

    std::vector<int> ref_top(topN), eng_top(topN);

    if (getenv("MIMO_FULL_LOGITS")) {
        FILE* lf = fopen(getenv("MIMO_FULL_LOGITS"), "w");
        if (lf) { for (int i = 0; i < model.cfg.vocab_size; i++) fprintf(lf, "%d %g\n", i, last_logits[i]); fclose(lf); }
    }
    ...
    printf("mimo engine top1=%d  ref top1=%d  top-%d overlap=%d/%d (need %d)\n",
