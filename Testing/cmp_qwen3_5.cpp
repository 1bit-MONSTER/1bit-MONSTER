// cmp_qwen3_5.cpp — Qwen3_5 text decoder e2e gate.
//
// usage: cmp_qwen3_5 <model_dir> <ids.txt> <hf_logits.npy> [topN] [min_overlap]
#include <cstdio>
#include <fstream>
#include <sstream>
#include <string>
...
int main(int argc, char** argv) {
    if (argc < 4) { printf("usage: cmp_qwen3_5 <model_dir> <ids.txt> <hf_logits.npy> [topN] [min_overlap]\n"); return 2; }
    ...
    Qwen3_5KVCache cache;
    int pos = 0;
    std::vector<float> last_logits;
    for (size_t i = 0; i < ids.size(); i++) {
        last_logits = qwen3_5_forward(model, ids[i], cache, pos);
    }

    if (getenv("Q35_FULL_LOGITS")) {
        FILE* lf = fopen(getenv("Q35_FULL_LOGITS"), "w");
    ...
    printf("q35 engine top1=%d  ref top1=%d  top-%d overlap=%d/%d (need %d)\n",
