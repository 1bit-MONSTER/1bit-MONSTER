#!/usr/bin/env python3
"""Mellum numpy reference (port of transformers modeling_mellum.py 5.14).

Validates the engine's mellum path on the tiny random checkpoint.
Run: python3 Testing/e2e_numpy_ref_mellum.py <model_dir> <ids.txt> <N>
...
def rope(q, k, pos, theta, partial=1.0):
    """RoPE with per-layer-type theta, optional partial rotary (dim = HD*partial)."""
    HD = q.shape[1]
    dim = int(HD * partial)
    ...
    def apply(x):
        x1 = x[:, :dim//2]; x2 = x[:, dim//2:dim]
        xr = np.concatenate([x1*cos - x2*sin, x1*sin + x2*cos], axis=-1)
        return np.concatenate([xr, x[:, dim:]], axis=-1)
...
def attention(hidden, l, w, cfg, pos, kv_k, kv_v):
    ...
    q, k = rope(q, k, pos, rp['rope_theta'], rp.get('partial_rotary_factor', 1.0))
