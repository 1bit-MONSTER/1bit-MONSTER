#!/usr/bin/env python3
"""ExaoneMoe numpy reference (port of transformers modeling_exaone_moe.py 5.14).

Validates the engine's exaone_moe path on the tiny random checkpoint.
Run: python3 Testing/e2e_numpy_ref_exaonemoe.py <model_dir> <ids.txt> <N>
...
def attention(hidden, l, w, cfg, pos, kv_k, kv_v):
    ...
    # per-head QK norm (always applied)
...
def moe(hidden, l, w, cfg):
    ...
    NG = cfg['n_group']; TG = cfg['topk_group']
    logits = hidden @ w[pfx + 'gate.weight'].T
    scores = 1.0 / (1.0 + np.exp(-logits))  # sigmoid
    bias = w[pfx + 'e_score_correction_bias']
    sc = scores + bias
    # group-limited selection
    group_scores = sc.reshape(NG, E // NG)
    gs = np.sort(group_scores, axis=-1)[:, -2:].sum(axis=-1)  # top2 per group
    group_idx = np.argsort(-gs)[:TG]
    mask = np.zeros(NG)
    mask[group_idx] = 1.0
    score_mask = np.repeat(mask, E // NG)
    sc_masked = np.where(score_mask > 0, sc, -np.inf)
    idx = np.argsort(-sc_masked)[:TOPK]
    rw = scores[idx]
