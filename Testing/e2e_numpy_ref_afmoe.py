#!/usr/bin/env python3
"""AfMoE numpy reference (port of transformers modeling_afmoe.py 5.14).

Validates the engine's afmoe path on the tiny random checkpoint.
Run: python3 Testing/e2e_numpy_ref_afmoe.py <model_dir> <ids.txt> <N>
...
def attention(hidden, l, w, cfg, pos, kv_k, kv_v):
    ...
    gate = hidden @ w[pfx + 'gate_proj.weight'].T  # [NH*HD]
    ...
    out = out * (1.0 / (1.0 + np.exp(-gate)))  # sigmoid gate
...
def moe(hidden, l, w, cfg):
    ...
    idx = np.argsort(-(scores + (bias if bias is not None else 0.0)))[:TOPK]
    ts = scores[idx]
    ts = ts / (ts.sum() + 1e-20)
    ts = ts * cfg['route_scale']
    out = np.zeros(hidden.shape[0])
    for ei, wt in zip(idx, ts):
        g = silu(hidden @ w[f'{pfx}experts.{ei}.gate_proj.weight'].T)
    ...
    shared = mlp(hidden, l, w, cfg, prefix='mlp.shared_experts.',
                 intermediate=FF * cfg.get('num_shared_experts', 1))
    return out + shared
...
def block(hidden, l, w, cfg, pos, kv_k, kv_v):
    ...
    x = rmsnorm(x, w[pfx + 'post_attention_layernorm.weight'], eps)
    hidden = hidden + x
    # mlp with dual norm
    x = rmsnorm(hidden, w[pfx + 'pre_mlp_layernorm.weight'], eps)
    if l >= cfg['num_dense_layers']:
        x = moe(x, l, w, cfg)
    else:
        x = mlp(x, l, w, cfg)
    x = rmsnorm(x, w[pfx + 'post_mlp_layernorm.weight'], eps)
    return hidden + x
