...
def auto_docstring(*a, **k):
    # supports both @auto_docstring and @auto_docstring(...)
    if len(a) == 1 and callable(a[0]) and not isinstance(a[0], str):
        return a[0]
    def deco(cls): return cls
    return deco
from mimo_cfg import MiMoV2FlashConfig
...
class MiMoV2FlashRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: MiMoV2FlashConfig, is_swa, device=None):
        ...
        # plain default rope (theta-based, no scaling) — matches the real
        # checkpoint's rope_type=default
        theta = getattr(config, 'rope_theta', 10000.0)
        hd = int(self.config.head_dim * config.partial_rotary_factor)
        import torch as _t
        inv_freq = 1.0 / (theta ** (_t.arange(0, hd, 2).float() / hd))
        self.attention_scaling = 1.0
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self.original_inv_freq = self.inv_freq

    @staticmethod
    def compute_default_rope_parameters(config=None, device=None, seq_len=None, **kw):
        import torch as _t
        return _t.zeros(1), 1.0
