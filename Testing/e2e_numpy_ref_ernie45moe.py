#!/usr/bin/env python3
"""Ernie4.5-MoE numpy reference (port of transformers modeling_ernie4_5_moe.py 5.14).

Validates the engine's ernie4_5_moe path on the tiny random checkpoint.
Run: python3 Testing/e2e_numpy_ref_ernie45moe.py <model_dir> <ids.txt> <N>
...
def moe(hidden, l, w, cfg):
    ...
    E = cfg.get('num_experts', cfg['moe_num_experts']); TOPK = cfg.get('num_experts_per_tok', cfg['moe_k'])
    FF = cfg['moe_intermediate_size']
    logits = hidden @ w[pfx + 'gate.weight'].T
    # softmax router + e_score_correction_bias, topk on bias-adjusted, norm /clamp(sum,min)
    e = np.exp(logits - logits.max())
    routing = e / e.sum()
    bias = w[pfx + 'moe_statics.e_score_correction_bias'][0]
    # torch.topk tie-break: equal values pick the HIGHER index. Replicate
    # with lexsort on (score, index) reversed.
    order = np.lexsort((np.arange(E), -(routing + bias)))[::-1]
    idx = order[:TOPK]
    rw = routing[idx]
    rw = rw / np.clip(rw.sum(), min=cfg['moe_norm_min'])
    out = np.zeros(hidden.shape[0])
    for ei, wt in zip(idx, rw):
        g = gelu(hidden @ w[f'{pfx}experts.{ei}.gate_proj.weight'].T)
        u = hidden @ w[f'{pfx}experts.{ei}.up_proj.weight'].T
        out += wt * ((g * u) @ w[f'{pfx}experts.{ei}.down_proj.weight'].T)
    shared = mlp(hidden, l, w, cfg, prefix='mlp.shared_experts.',
                 intermediate=FF * cfg.get('moe_num_shared_experts', 0))
    return out + shared

def is_moe_layer(cfg, l):
    return ((l + 1) % cfg['moe_layer_interval'] == 0
            and l >= cfg['moe_layer_start_index'] and l <= cfg['moe_layer_end_index'])
