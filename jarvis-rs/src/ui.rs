//! Static chat UI. Port of jarvis/ui.py.
//!
//! This is a Rust raw string, which — unlike the Python triple-quoted
//! string it replaces — never interprets backslash escapes at all. That
//! permanently rules out the class of bug just fixed in jarvis/ui.py, where
//! Python's own string parser ate `\uXXXX` and `\n` escapes meant for the
//! embedded JS before they ever reached the browser (crashing every
//! request, then breaking every button). The 🔊/🎤 emoji are written here
//! as literal UTF-8 characters instead of escape sequences for the same
//! reason — nothing to misinterpret.
