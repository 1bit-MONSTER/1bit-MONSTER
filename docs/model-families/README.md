...
1bit auto-detects **566 architecture tokens / 1,946 HF arch strings** from GGUF/1BP/safetensors headers — no config files, no per-model glue code. Point the engine at a model and run. Coverage is measured against a full HF census: **321,611 / 321,611 arch-bearing text-gen checkpoints (100.00%) map to an engine token** — see the [models SSOT](../wiki/models.md).
