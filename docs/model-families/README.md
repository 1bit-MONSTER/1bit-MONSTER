...
1bit auto-detects **569 architecture tokens / 2,044 HF arch strings** from GGUF/1BP/safetensors headers — no config files, no per-model glue code. Point the engine at a model and run. Coverage is measured against a full HF census: **324,712 / 324,863 arch-bearing text-gen checkpoints (99.95%) map to an engine token** — see the [models SSOT](../wiki/models.md).
