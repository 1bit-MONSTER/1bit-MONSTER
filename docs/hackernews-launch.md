...
AMD shipped a 50 TOPS XDNA 2 NPU locked behind a closed-source runtime (FastFlowLM): 22 proprietary .so files, 209 xclbin bitstreams, zero documentation. I took it apart in 4 days — the firmware is RSA-2048 signed, so the mailbox protocol had to be decoded by hand with a disassembler, ftrace and bpftrace — and replaced the whole stack with open C++. The unedited session logs are in the repo: https://github.com/1bit-MONSTER/1bit-MONSTER/blob/main/docs/journey.md
...
* JARVIS: a fully-local voice pipeline (mic → STT → LLM → TTS → speaker) as one subcommand.
...
Honest caveats: it's a young project — NPU support targets Strix Halo-class XDNA2 SKUs today; the GPU/CPU paths are the battle-tested ones. Issues, feedback and contributions welcome (MIT).
