...
6. **Bespoke re-validation:** zamba2, mamba, deepseek, kimi, whisper — existing backends; one e2e per family (SSM/MLA/STT paths need their own oracles; whisper via transcript compare).
   **deepseek-mla DONE 2026-08-15 (validated / numpy-exact tier):** the MLA backend (src/deepseek.cpp) was DEAD UNWIRED CODE written for a different GGUF variant — rewritten to match the real DeepSeek-V2-Lite Q8 GGUF (mradermacher). Real bugs found & fixed: missing q_a/kv_a latent RMSNorms, expert block-offset bug (every block wrote the first 32 slots), down-projection transposed read, chunk→ADJACENT rope pairing (llama.cpp LLAMA_ROPE_TYPE_NORM), exps layout [A innermost / expert outer], fused 2-shared experts, uncompressed attn_q (q_lora_rank=None), dense layer 0 (DI=10944), untied lm_head, scale 1/sqrt(192). Verified: engine ≡ f32 numpy port of modeling_deepseek.py at all layers; correct next-token behavior (fox/jumps/over/the/lazy/dog/.); router selections identical to llama.cpp; periodic-prompt generation matches llama-completion 19/20. Known gap: on weak random-text positions (PPL ~21 degenerate base) the argmaxes diverge from llama with non-trivial margins — an unresolved small detail, honestly not claimed as clean 20/20. Memory: experts Q8→f16 ~29GB (vs 63GB f32). Harness: `Testing/e2e_deepseek.cpp`.
7. **gpt-oss (OpenAI, MXFP4-packed MoE) — DONE 2026-08-14: engine 20/20 generated tokens == numpy reference** (port of authoritative modeling_gpt_oss.py) on the REAL openai/gpt-oss-20b checkpoint (13.4GB packed, in `/tmp/onebit-e2e/gptoss`), full logits max|diff| 6.7e-5, top-8 EXACT. The memory-block (dequantized ~105GB fp32) is sidestepped by keeping the MoE **packed U8 blocks+scales in RAM and dequantizing only the selected 4 experts per token** (~17GB total, no torch reference needed). Quirks landed: YARN rope (theta 150000, factor 32, beta 32/1, orig_max 4096 — the ramp runs over FREQ indices arange(dim//2), an easy 2x), attention sinks (per-head learned logit cat to scores before softmax, dropped after), router bias, MXFP4 decode (FP4 e2m1, value = FP4[nibble]·2^(scale−127), low nibble→even/high→odd, 32 vals/block), interleaved gate/up rows, gate=min(g,7)·sigmoid(1.702g), **gated=(up+1)·glu** (the `+1` was missing in the first pass), top-4 softmax gating (softmax over top-k only), untied lm_head, head_dim 64, odd-safe rope pairing. Validation harness: `Testing/e2e_numpy_ref_gptoss.py` (reference) + `e2e_seq_gen` (engine). Sliding-window (128) attention now MASKED (2026-08-15, commit 70dcc040): scores[t] = -inf for t < pos-127 on even layers — mirrors the numpy ref's t0 = max(0, pos-window+1) exactly. Long-context e2e gate pending the re-downloaded fixture (13.4GB, deleted in a disk cleanup).
...
9. **bloom (bigscience) — DONE 2026-08-15: 20/20 generated tokens identical to torch** (transformers 5.14 bloom, f32; bigscience/bloom-560m, /tmp/onebit-e2e/bloom), full logits max|diff| 7.6e-4 corr 1.0. Quirks landed: fused query_key_value HEAD-INTERLEAVED (per-head [q,k,v] groups — modeling_bloom._reshape view(seq,heads,3,hd); a naive [q|k|v] row split produces scrambled attention), LayerNorm w+bias (input + post_attention, SEQUENTIAL), non-gated tanh-gelu FFN w/ bias, LINEAR ALiBi (same slope table as step1's sqrt variant, linear distance), word_embeddings_layernorm (embedding LayerNorm), tied lm_head, config keys n_embed/n_head/n_layer/layer_norm_epsilon. The 1,086-checkpoint class the docs wrongly grouped with encoder-decoders is now gated.
10. **whisper (STT) — DONE 2026-08-15: transcript EXACTLY matches the ground truth** (openai/whisper-tiny + the JFK clip: 'And so my fellow Americans ask not what your country can do for you ask what you can do for your country', EOT at step 22). The backend was DEAD CODE — the port pass fixed: HF-style GGUF loader name-mapping (incl. the cross_attn->encoder_attn ordering bug where 'attn_ln.' matched inside 'cross_attn_ln.'), WAV fmt-chunk skip, self/cross-attn buffer overflows, conv1d padding (1500->748 frames), the mel (GGUF mel_filters [80x201] + periodic hann + center reflect padding + power=2 + log10 + max-8 clamp + (x+4)/4 + per-row 30s pad — corr 0.9999 vs the HF feature extractor), learned encoder position embedding, encoder + decoder residual overwrites (the normed input clobbered the residual), the decoder prefix [50258 SOT, 50259 en, 50359 transcribe, 50363 no-timestamps] — the old [50257, task, lang] order produced repeated blanks — and EOS=50257 (not 50256). Verified: encoder corr 0.944 vs torch, decoder step corr 0.992, first tokens == torch generate. Gate: manifest transcript match (PASSES). Harness: `Testing/e2e_whisper.cpp`.
...
- **MoE CPU validation** — gptoss MoE validated via the packed per-expert numpy oracle (2026-08-14); **deepseek MLA validated 2026-08-15** (V2-Lite, numpy-exact tier — see Phase 2 #6); qwen3moe deferred to the engine tokenizer (htok workstream). Biggest remaining causal-LM class: **DeepSeek-V3** (the V2 MLA path ports directly; V3 needs the sigmoid-gated 256-expert config + group-limited top-k).
