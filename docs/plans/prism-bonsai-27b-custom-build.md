...
| P3.2i | ✅ **dispatch floor measured; two mechanisms retired (2026-09-18 15:22).** Direct measurement (trivial-kernel loop, no new instrument): 1.826 µs/launch trivial, 1.984 µs realistic ⟹ 1541 dispatches = **2.8-3.1 ms of host time** against 29.5 ms of GPU work per token, so launch cost is **overlapped, not additive** ⟹ the dispatch story is dead as a material cost and the fusion win was slightly less GPU work, not fewer launches. Peer also corrected their own copy figure (418 dispatches/token = 27%, not 1452/94% — a mis-division); the earlier advice to attack the copies is withdrawn, and so is my part in it (I checked the ratio, never the provenance of the numerator). **Q1_0 bounds now firm:** GEMV floor 18.1 ms (3.78 GB payload at ~210 GB/s triad) vs 21.2 measured = 85% of the floor rate; gate needs 23.81 ms ⟹ non-GEMV must fall ~32% if the GEMV reaches its floor, or ~69% if it does not. **Both halves are required**: GEMV-at-floor with non-GEMV untouched is 37.7 tok/s (short), and deleting the largest non-GEMV kernel outright is 37.8 tok/s (short). Next: `gdn_recurrence` (3.11 ms, the largest) first, then rmsnorm (1.16) and fused-fwht (~1), with the GEMV chase continuing in parallel — neither alone closes Q1_0. |
...
| P3.2k | ✅ **gdn_recurrence split WINS in isolation (5baa93ff3, 2026-09-18 15:25) — the neutral call is reversed.** Isolated A/B (same geometry, both kernels built from this tree, alternated via git stash): **46.143 → 28.452 µs/layer = 2.215 → 1.366 ms/token, 1.62×, 0.85 ms/token saved.** Correctness unchanged (oracle 5/5, compare_gen 11/11, GDN gates pass) and per-element fp32 arithmetic bit-identical because the per-thread traversal order is unchanged — which is why landing a restructure was acceptable. **Method rule recorded:** the aggregate read 30.2 vs 29.6-31.5, i.e. inside the noise band, so the aggregate alone would have discarded a real optimisation ⟹ *isolated A/B is the instrument for kernel claims, the aggregate is the instrument for gate claims*, and no gate moves on kernel-level evidence (gate state recorded UNCHANGED: Q1_0 32-34 / PTQ1_0 23-24 / PQ2_0 24 vs 42/27/22, PQ2_0 MET). **In-situ tax now has two data points**: gdn +40% in situ (3.11 vs 2.215) and the GEMV +17% over its memory floor (21.2 vs 18.1) — one problem, not two, matching the shape finding from the other direction; the loop costs 17-40% on every kernel in it. **Chase arithmetic sharpened:** gap 5.79 ms − 0.85 addressed = 4.94 ms; predicted non-GEMV 7.55 ms (UNMEASURED) → 34.8 tok/s, and even with the GEMV at its 18.1 ms floor → 39.0 tok/s, still short ⟹ clearing Q1_0 needs non-GEMV at 5.71 ms *with* the GEMV at floor. Next decisive number: **post-split in-situ gdn** (does the isolated gain survive the loop where the tax lives?); remaining named non-GEMV: rmsnorm 1.16, fused fwht ~1.0. |
| P3.2l | ✅ **gate integrity: a skipped gate is not a green gate (2026-09-18 16:50).** Peer self-reported the defect class: an optional-build failure printed a parenthetical and continued, so **P3.3 (full device forward vs fork oracle) was SKIPPED, not passing, through several green reports** — including my host-only runs, which skipped every device gate silently through a guard that printed nothing. Fix (structural): the runner counts passes and skips, prints `gates: passed=N failed=M skipped=K`, and **cannot claim an unqualified green over a skip** (undeclared skip ⇒ "NOT A GREEN RUN" + non-zero exit; declared host-only skips are named). New `tests/prism/check_suite_log.py` enforces the same at log level with a 7-fixture self-test. **Verified by re-creating the defect**: with the dp4a TU removed from the build list the suite reports `passed=47 failed=0 skipped=1`, names the cause, and exits 1; on the real tree P3.3 now PASSES on all three packs. Also observed and NOT reproduced: one run under load reported `failed=1`; three subsequent runs were clean at `passed=53 failed=0 skipped=0`, and the failing gate's name is lost because I deleted that log before reading it (my process error, recorded as an unexplained flake to watch). |
| P3.2m | ◐ **three-kernel window (15:45:05): wall unchanged; Q1_0 is now GEMV-limited (2026-09-18).** Valid window both sides (triad 213.0 → 214.3): **Q1_0 34 / PTQ1_0 24 / PQ2_0 24 tok/s vs 42/27/22 = 81% / 89% / 109%, PQ2_0 MET**; oracle 5/5, compare_gen 11/11 in-window. **No improvement claimed**: Q1_0 wall read 29.6 ms here against 29.6 ms in the earlier load-2.2 window, so the three measured kernel wins (GDN 5baa93ff3, rmsnorm 1ca20bc07, FWHT 82efae438) did not appear on a box at load 11.5 — the tax absorbed them, and the two are not separable on this evidence. Firm result from the round = the in-situ attribution: **GEMV ~20.0 ms, non-GEMV ~4.7-5 ms, wall 29.6**. **This supersedes my two-chase frame**: non-GEMV fell 8.4 → ~4.7-5 ms, and GEMV floor (16.9 ms for 3.60 GB at 213 GB/s) + current non-GEMV = 21.6-21.9 ms, which clears the 23.81 ms gate ⟹ **Q1_0 is limited by the GEMV reaching its memory bound and nothing else**. Caveat: that projection is built from overlapping in-situ durations (they sum 24.8 ms against a 29.6 ms wall), so it is arithmetic, not a gate measurement — only a window moves a gate. **Five GEMV hypotheses retired by measurement** (quant, dispatch, in-situ tax, lane-tail/r4, LDS staging — the last at 32 GB/s vs 220 for r1); the survivor is **an unpack-ALU limit, not bandwidth and not tiling**, recorded as the surviving hypothesis, not a finding. Peer moves to P5 (Vulkan/ZINC third column) and P4 (NPU arm) for breadth while that question stays open. Also open: the pack size reads 3.60 GB in the peer's floor arithmetic against 3.78 GB payload file / 3.782 GB payload in this lane's records — gate fractions are size-independent (81/89/109%) but effective-GB/s rows depend on it. |
...
|---|---|---|---|---|
| Bonsai-27B 1-bit (Q1_0 gguf) | 3.78 GB payload | ~53 tok/s | **≥ 42 tok/s** | **1.5× the fork's 28.8** |
