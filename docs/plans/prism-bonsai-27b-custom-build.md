...
**Owner:** @agent-dddf9e · **Date:** 2026-09-18 · **Status:** **P0, P1, P2 complete — P3 (HIP)
active.** Artifacts acquired and locked against Prism's own runtime; 1BP v5 container +
converter byte-exact against every source GGUF (23.9 GB payload compared, 0 mismatches); full
64-layer CPU forward agrees with the fork's own next-token oracle on all three packs (5/5 exact
top-1 each). Evidence: `docs/research/prism-bonsai-27b/` and `tests/prism/` (`run_prism_tests.sh`
→ ALL PRISM GATES PASSED).
...
| # | Task |
|---|---|
| P3.1 | Wire `hadamard_rotate_butterfly.hip` into CMake; generalize B=128 → B∈{512,1024,2048,4096} with **explicit signs**, bf16/fp16 paths, in-place; inverse variant |
| P3.2 | ◐ **first increment done — HIP GEMV for all three Prism layouts, bit-accurate on device.** `tests/prism/prism_gemv_hip.hip`: decodes the verbatim block-128 payloads in-kernel (Q1_0 18 B, PQ2_0 34 B, PTQ1_0 28 B with its non-positional trit order) and computes `y = W·x`; verified against the CPU reference (`prism::dequant_flat` + f64 dot) — **corr = 1.000000000, max|Δ| ≈ 2e-7** for all three packings on `blk.0.ffn_gate.weight` (17408×5120). Measured on gfx1151 (weight bytes / time): **v1 simple 15.5–24.2 GB/s; v2 templated decoder 19.3–31.8; v3 warp-per-row + shuffle reduction 22.1 (PTQ1_0) / 23.4 (Q1_0) / 35.9 (PQ2_0) GB/s = 11–18% of the 201 GB/s ceiling**; a **dummy control** with the identical memory pattern but no decode reaches 33.8–59.7 GB/s. Diagnosis, from that control rather than from guessing: ~40% of the cost is the decode, and the larger share is the loop shape (one element per iteration), not DRAM — so the next kernel decodes **one byte → 4–8 codes** with `float4` x loads, stages x in LDS, and needs a PTQ1_0-specific cooperative scheme (its byte↔element mapping is not positional). Gate: the parity check is now in `run_prism_tests.sh` for all three models |
| P3.3 | Fuse: `fwht(x) → {qkv|z|gate/up}` reuse (one rotation per input slab, 5120-wide), l2norm + conv1d + gated-delta recurrence kernel, RMSNormGated, partial-MRoPE (interleaved) + q/k norm + swish output gate |
| P3.4 | KV: full-attn layers only (16 of 64) with 4 kv heads × 256 head_dim; GDN recurrent state 48×128×128 fp32 per layer (~1.5 GB total, constant, no growth with context); 262K-context plan separate from v1 (start 32K) |
| P3.5 | Wire into `backend_hip_1bp.cpp` — it currently **refuses** hybrid SSM models (L291-301); add `RCPP_ARCH_QWEN35` graph + a capability gate that names the missing piece instead of failing blind |
...
| # | Task |
|---|---|
| P4.1 | **Bridge first**: extend `ternary_npu_bridge.h` for g128 B1/TQ2 tensors + the Hadamard-fused activation path; use existing INT8 xclbins |
