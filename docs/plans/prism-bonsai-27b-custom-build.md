...
| P3.3 | ◐ **kernels done (2026-09-18); fusion/integration open.** `kernels/prism_gdn.hip` adds the two genuinely-new GDN kernels: `prism_conv1d_silu_f32` (causal depthwise conv1d k=4 + silu + rolling 3-tap state, one thread/channel) and `prism_gdn_recurrence_f32` (per v-head: fp32 state, decay, delta rule, `c = sᵀq/√HK`, gated RMSNorm over HV), plus `prism_ssmout_perm_f32` for the folded base. Gate `tests/prism/test_prism_gdn_hip.hip` (in `run_prism_tests.sh`): conv1d+state max dev 1.0e-07, recurrence+norm core 3.6e-07 / state 3.6e-08, perm exact, at the real NK=16 NV=48 HK=HV=128. **Open:** the fwht→{qkv|z|gate/up} one-rotation-per-slab sequencing (P3.1+P3.2 launchers already support it), RMSNormGated fusion, partial-MRoPE + q/k norm + swish output gate for the 16 full-attn layers, and a single device-side per-token driver |
