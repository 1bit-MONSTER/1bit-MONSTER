...
| P3.5 | ◐ **scoped (2026-09-18).** `src/backend_hip_1bp.cpp` already detects `RCPP_ARCH_QWEN35` (cfg.arch, GGUF arch `qwen35moe`, or the `blk.0.ssm_a` signature) but the wired path is **Q4NX-only for qwen35moe** (GDN dims ssm_a{32}, alpha/beta{2048,32}, conv1d{4,8192}, out{4096,2048}) and refuses other quants. The Prism packs are Q1_0/PQ2_0/PTQ1_0 with very different geometry (GDN qkv 10240, NV=48, HK=HV=128, CD=10240, VD=6144) plus the folded transform, so P3.5 needs a **new Prism branch**: recognise `__onebp_ext_prism_transform`, size the GDN/attention scratch from the 1BP header, dispatch the tile GEMV (P3.2b) + FWHT (P3.1) + GDN/attn/ops kernels (P3.3), and **fail closed** when a folded pack is served without the transform. Validate through `prism_forward_hip` first, then the engine. |
