...
**Owner:** @agent-dddf9e · **Date:** 2026-09-18 · **Status:** **P0–P2 complete; P3 in progress (P3.1 signed FWHT + P3.2 GEMV done).** Artifacts acquired and locked against Prism's own runtime; 1BP v5 container + converter byte-exact against every source GGUF (23.9 GB payload compared, 0 mismatches); full 64-layer CPU forward agrees with the fork's own next-token oracle on all three packs (5/5 exact top-1 each) **and** with the in-repo streaming numpy reference per layer (cosine ≥ 0.999, all 64 layers, all three packs). Evidence: `docs/research/prism-bonsai-27b/` and `tests/prism/` (`run_prism_tests.sh` → ALL PRISM GATES PASSED).
...
| P3.3 | ◐ **kernels done (2026-09-18); driver/integration open.** ① `kernels/prism_gdn.hip`: `prism_conv1d_silu_f32` (causal depthwise conv1d k=4 + silu + rolling 3-tap state), `prism_gdn_recurrence_f32` (per v-head fp32 state, decay, delta rule, `c=sᵀq/√HK`, gated RMSNorm), `prism_ssmout_perm_f32`. Gate: conv 1.0e-07, recurrence+norm core 3.6e-07 / state 3.6e-08, perm exact. ② `kernels/prism_attn.hip`: `prism_qk_norm_rope_f32` (per-head plain RMSNorm + partial RoPE, half-split **and** interleaved) and `prism_gqa_decode_f32` (single-token flash decode, one warp/q-head, online softmax, sigmoid **or** silu output gate). Gate: max dev 1.3e-06 / 1.2e-07. **Open:** the one-rotation-per-slab fwht→GEMV sequencing, the per-head l2norm + g/β elementwise kernels, and a single device-side per-token driver. ③ `kernels/prism_ops.hip`: per-head l2norm, plain RMSNorm (llama.cpp convention), g/β, residual add — gate max dev 1.5e-08 / 2.4e-07 / 1.2e-07 / 0. |
