...
every page walked) counts **412,785 text-gen checkpoints**, 326,174 with an
`architectures` field (86,611 have none). After excluding ~4,730 out-of-scope
encoder-decoders, **325,946 / 326,174 arch-bearing text-gen checkpoints
(99.93%) map to an engine token** (class-name + model_type two-step dispatch);
