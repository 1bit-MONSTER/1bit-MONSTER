...
Real-checkpoint census of the HuggingFace hub (`/api/models?pipeline_tag=text-generation&config=true`). **FULL 100% enumeration 2026-08-15** (`Testing/census_full_summary.json`, every page walked): **399,220 text-generation checkpoints** (the earlier 220,049 figure was a 250-page sample — it undercounted by 1.81x), of which **322,010 declare an `architectures` field** (77,210 have none — unmappable). **302,330 / 322,029 arch-bearing checkpoints (93.88%) map to an engine token** (the `rcpp_arch_from_string` registry — number regenerated by `Testing/census_coverage.py`, which measures the ACTUAL committed mapping function, not a script-side copy). **Two-step dispatch (2026-08-15, see [modular-arch-registry-extraction.md](../research/modular-arch-registry-extraction.md)):** class name → token, then UNKNOWN class → HF `model_type` → token. On the full 399,376-model census: class-name 76.0%, +model_type 77.0% (arch-bearing 94.57%). The 23 manifest families (22 validated with e2e generation gates) cover the bulk of it; the gap to the registry number is the loadable-but-unvalidated tokens (qwen35 ~4,300, deepseek v2/v3 ~780, VLM/SSM tails). The validated families (torch/numpy/llama.cpp-exact generation gates; `Testing/bringup_runner.sh` is now manifest-driven with per-family `gate` commands):
...
`architectures` field. **304,534 / 322,029 (94.57%) map to an engine token**
(class-name + model_type two-step dispatch); the 23 manifest families (22
validated) gate the bulk of it. Per-class counts
...
~302k checkpoints was individually run. The full per-arch table is in
