...
every page walked) counts **412,266 text-gen checkpoints**, 325,968 with an
`architectures` field (86,298 have none). After excluding ~4,730 out-of-scope
encoder-decoders, **325,761 / 325,968 arch-bearing text-gen checkpoints
(99.94%) map to an engine token** (class-name + model_type two-step dispatch);
