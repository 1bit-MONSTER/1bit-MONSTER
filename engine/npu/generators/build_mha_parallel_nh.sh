...
# native bf16 MHA generator n1_mha_parallel_nh.py.
#
...
#
# Usage: bash build_mha_parallel_nh.sh <N_chunk> <C_chunks> <NH> [depth] [outdir]
#   e.g. bash build_mha_chunked.sh 128 2            # depth defaults to 2
#        bash build_mha_chunked.sh 64 4 4 ~/npu-build/mha_n64_c4_d4
# Outputs: design.mlir, mha_par.xclbin, mha_par_insts.txt, logs.
set -euo pipefail

N="${1:?usage: build_mha_parallel_nh.sh <N_chunk> <C_chunks> <NH> [depth] [outdir]}"
C="${2:?usage: build_mha_parallel_nh.sh <N_chunk> <C_chunks> <NH> [depth] [outdir]}"

NH="${3:-1}"; DEPTH="${4:-2}"; OUT="${5:-$HOME/npu-build/mha_par_nh${NH}_n${N}_c${C}_d${DEPTH}}"
HD=128
M=16
...
echo "== generator: n1_mha_parallel_nh.py -M $M -N $N -C $C -HD $HD -D $DEPTH -NH $NH ${VDIRECT:+--v-direct }${VSD:+--vs-depth $VSD}"
"$PY" "$G/n1_mha_parallel_nh.py" -M "$M" -N "$N" -C "$C" -HD "$HD" -D "$DEPTH" -NH "$NH" \
...
if ! "$AIECC" --peano="$P" --aietools="$AIETOOLS" \
      ...
      design.mlir -o mha_par.xclbin >aiecc.log 2>&1; then
  echo "== AIECC FAILED"; tail -25 aiecc.log; exit 1
fi
[ -f main.xclbin ] && cp -f main.xclbin mha_par.xclbin
[ -f main_seq.bin ] && cp -f main_seq.bin mha_par_insts.txt
[ -f mha_par.xclbin ] || { echo "== NO XCLBIN"; tail -20 aiecc.log; exit 1; }
echo "== OK: $OUT/mha_par.xclbin ($(stat -c%s mha_par.xclbin) B), mha_par_insts.txt ($(stat -c%s mha_par_insts.txt 2>/dev/null || echo 0) B)"
