...
# native bf16 MHA generator n1_mha_chunked_nh.py.
#
...
#
# Usage: bash build_mha_chunked_nh.sh <N_chunk> <C_chunks> <NH> [depth] [outdir]
...
N="${1:?usage: build_mha_chunked_nh.sh <N_chunk> <C_chunks> <NH> [depth] [outdir]}"
C="${2:?usage: build_mha_chunked_nh.sh <N_chunk> <C_chunks> <NH> [depth] [outdir]}"

NH="${3:-1}"; DEPTH="${4:-2}"; OUT="${5:-$HOME/npu-build/mha_nh${NH}_n${N}_c${C}_d${DEPTH}}"
HD=128
M=16
...
echo "== generator: n1_mha_chunked_nh.py -M $M -N $N -C $C -HD $HD -D $DEPTH -NH $NH ${VDIRECT:+--v-direct }${VSD:+--vs-depth $VSD}"
"$PY" "$G/n1_mha_chunked_nh.py" -M "$M" -N "$N" -C "$C" -HD "$HD" -D "$DEPTH" -NH "$NH" \
