...
# (matmul_i8_i32_wide, n=N_D), then the 8 partials are summed in ONE cascade
# pass (cascade_reduce_{first,mid,last}_i32_wide); col 7 writes the FULL
# (8×N_D) C2. No h2 DDR round-trip → no cross-shim visibility race.
#
# N_D must fit L1 as (8×N_D) int32 = N_D*4 KB (32 KB @ N_D=1024). N_D=2048
# would be 64 KB (too big) — so the default is N_D=1024.
set -euo pipefail
G="$(cd "$(dirname "$0")" && pwd)"
...
# 2. WIDE D kernel object (n=N_D): matmul_i8_i32_wide + cascade_reduce_*_wide
$P/bin/clang++ --target=aie2p-none-unknown-elf --std=c++20 -O2 \
    -DDIM_M=8 -DDIM_K=64 -DDIM_N=128 -Di8_i32_ONLY -DM8_VECTORIZED \
    -DWIDE_DIM_N="$N_D" \
    -isystem $P/include/c++/v1 \
...
echo "OK: $(ls -la "$G/../xclbins/final_cascade_fused.xclbin" | awk '{print $5}') B xclbin + insts (N_D=$N_D)"
