...
def main():
    ...
    p.add_argument("-NH", type=int, default=1, help="heads (head-outer/chunk-inner loop)")
    ...
    with mlir_mod_ctx() as ctx:
        mha(a.M, a.N, a.C, a.HD, a.depth, a.v_direct, a.vs_depth, a.async_dma, a.NH)
        print(ctx.module)


def mha(M, N, C, HD, DEPTH=2, VDIRECT=False, VSD=1, ASYNCDMA=False, NH=1):
    ...
    def device_body():
        ...
        def qk_body():
            for _h in range(NH):   # Python-unrolled: distinct static buffer indices per head
                for _ in range_(C):
        ...
        def sm_body():
            for _h in range(NH):   # Python-unrolled: distinct static buffer indices per head
                # Per-head fresh running state: the softmax statics (m,l) persist
                # for the xclbin's life, so head h must not inherit head h-1's.
                softmax_reset()
                for _ in range_(C):
        ...
        def rs_body():
            for _h in range(NH):   # Python-unrolled: distinct static buffer indices per head
                # Fresh O accumulator per head (the static persists in the xclbin).
                combine_reset()
                for _ in range_(C):
        ...
        def seq(QK_all, V_all, O):
            ...
            for h in range(NH):
                for c in range(C):
                    base_qk = (h * C + c) * (M * HD + HD * N)
                    qt = shim_dma_single_bd_task(QK_s, QK_all, offset=base_qk,
                                                 sizes=[1, 1, M, HD], strides=[1, 1, HD, 1], issue_token=True)
                    dma_start_task(qt); pend.append(qt)
                    ktt = shim_dma_single_bd_task(QK_s, QK_all, offset=base_qk + M * HD,
                                                  sizes=[HD // 8, N // 8, 8, 8], strides=[8 * N, 8, N, 1],
                                                  issue_token=True)
                    dma_start_task(ktt); pend.append(ktt)
                    vt = shim_dma_single_bd_task(V_c if VDIRECT else V_s,
                                                 V_all, offset=(h * C + c) * N * HD,
                ...
                while pend:
                    dma_await_task(pend[0]); dma_free_task(pend[0]); pend.pop(0)
                # one output per head
                ot = shim_dma_single_bd_task(O_s, O, offset=h * M * HD,
                                             sizes=[M // 4, HD // 8, 4, 8],
