#!/usr/bin/env python3
#
# n1_core_fused_gu_silu_d_p2.py — D GEMM stage only (issue #1775 split).
#
# P2 of the split two-launch fused decode: GEMM2 (down) with A = h2 read
# from bo4 (written by the P1 launch, host-synced). Separate launch from P1
# so the host can place a visibility barrier between the h2 S2MM writeback
# and this D-phase read — eliminating the cross-shim DDR write→read race
# that made the single-launch fused decode nondeterministic (#1775).
#
...
def main():
    ...
    with mlir_mod_ctx() as ctx:
        my_fused_p2(args.M, args.K, args.N_GU, args.N_D, args.m, args.k, args.n,
                 args.cols, args.batch_size)
        print(ctx.module)


def my_fused_p2(M, K, N_GU, N_D, m, k, n, n_aie_cols=8, BATCH_SIZE=2):
    dtype_in = np.int8
    dtype_out = np.int32
