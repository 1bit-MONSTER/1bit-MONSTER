    ...
    auto forward = [&](int tok, int pos) -> int {
        ...
        for (int l = 0; l < NC; l++) {
            ...
            if (l % 2 == 0) {
                ...
                // Q/K/V1/V2 projections: each output row is an independent
                // dot product over H, so parallelizing over the row index keeps
                // every reduction sequential (bit-identical to the scalar loop).
