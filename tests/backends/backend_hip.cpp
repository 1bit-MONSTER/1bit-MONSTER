...
public:
    ...
    /// Print VRAM budget estimate and warn if the model likely won't fit.
    void check_vram_budget(const ModelConfig& cfg) {
        ...
        double fp16_mb = (double)V * H * 2.0 / (1024*1024);          // embedding
        fp16_mb += (double)L * cfg.max_seq_len * KD * 2.0 / (1024*1024);  // KV cache
        fp16_mb += 32.0;  // scratch
        // per-layer weights (QKV + O)
        ...
        if (fp16_mb > vram_total_mb_ * 1.1) {
            fprintf(stderr, "  ⚠️  Model too large for GPU VRAM even with Q4_K "
            ...
            fprintf(stderr, "  ⚠️  fp16 weights won't fit in VRAM (need ~%.0f MB, "
    ...
    bool load_gguf_model(const ModelConfig& cfg) {
        ...
        // The GGUF names may not match (Q2_0 models use different names),
        ...
        if (L > 0 && layers_[0].nw == nullptr) {
            fprintf(stderr, "  HIP: GGUF load failed — no per-layer tensors matched "
                    "(wrong quantization format? Q2_0 not supported, use Q4_K).\n");
            unload_model();
            return false;
        }
        embed_loaded_ = true;
