#!/usr/bin/env python3
"""gen_docs_site.py — build the 1bit.monster/docs static documentation site.
...
(function(){
  ...
  var B=(typeof DOCS_BASE!=='undefined')?DOCS_BASE:'';
  xhr.open('GET',B+'search.json');
  xhr.onload=function(){
  ...
  function show(list){
    ...
    list.slice(0,12).forEach(function(r){
      h+='<a href="'+B+r.url+'"><div class="t">'+esc(r.title)+'</div><div class="m">'+esc(r.lead)+'</div></a>';
    });
...
def page_shell(out: Path, *, title: str, lead: str, breadcrumb: str, nav_groups: dict,
               active_url: str, body: str, site_name: str, cur_dir: str,
               base_url: str, base_path: str) -> None:
...
<link rel="canonical" href="{html_mod.escape(base_url + active_url.lstrip('/'))}"/>
<style>{CSS}</style>
</head>
<body>
...
def main() -> int:
    ...
    ap.add_argument("--base-url", default="https://1bit.monster/docs/",
                    help="absolute base URL where the docs are mounted (used for canonical/sitemap)")
    args = ap.parse_args()
    ...
    # redirects live here.
    redirects = [
        "# Cloudflare Pages redirect rules for 1bit.monster/docs",
