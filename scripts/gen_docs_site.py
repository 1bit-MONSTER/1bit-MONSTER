...
def main() -> int:
    ...
    for rel, group, override in CURATED:
        ...
        # breadcrumb: link to group landing; for README no breadcrumb
        ...
        nav_groups.setdefault(group, []).append({
            "url": out_rel, "title": title, "rel": rel,
        })

        # search entry (url must be root-absolute so results work on any page)
        plain = html_mod.unescape(re.sub(r"<[^>]+>", " ", body))
        plain = re.sub(r"\s+", " ", plain).strip()
        search_entries.append({"title": title, "url": "/" + out_rel,
                               "lead": lead, "text": plain[:20000]})

        # clean-url rewrite: /path -> /path.html (and /section -> index.html)
        if out_rel == "index.html":
            continue
        if out_rel.endswith("index.html"):
            section = out_rel[: -len("index.html")].rstrip("/")
            redirect_lines.append(f"/{section}  /{out_rel}  200")
        else:
            clean = out_rel[:-len(".html")]
            redirect_lines.append(f"/{clean}  /{out_rel}  200")

    # sort nav entries within each group by curation order
    ...
    # Write _redirects (clean URLs + convenience)
    redirects = [
    ...
    print(f"search entries: {len(search_entries)}; redirect rules: {len(redirect_lines)+3}")
    return 0
