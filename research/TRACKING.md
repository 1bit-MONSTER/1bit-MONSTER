...
| Item | State | Evidence |
|------|-------|----------|
| Prism ML Bonsai 27B (1BP v5, 4 packs) | **P0-P2 closed; P3 kernels + engine serving done, tok/s gate deliberately unmeasured** — byte-exact verbatim container, both the CPU floor and the full device forward match the fork's own generated tokens, tile GEMV at roughly half the device triad; the gate number itself needs a quiet box (peer NPU engines were live); **P6 three-substrate PPL on one 12599-id stream** — our Q1_0 and PQ2_0 PPL match the fork oracle within 0.3%, PTQ1_0 is 14.9% worse (quality gap recorded, mechanism open); HRX/Loom column blocked on an unsupported op with evidence [3-packs | 1bp+GGUF | HIP PrismEngine vs Prism fork | strixhalo-unknown | 6138 | wiki.test.raw slice200 | 2026-09-19] | `tests/prism/PRISM_RESULTS.md` — every number tagged, enforced by `tests/prism/check_honesty_tags.py`; plan `docs/plans/prism-bonsai-27b-custom-build.md` |
| P0.1 NPU IO_PAGE_FAULT path | **not blocking any more** — NPU attention runs end-to-end at 1k ctx (~147 ms of attention for a 28-layer dense Qwen3 vs ~14 s CPU reference), served by the **embedded captured** ELF. The storm itself is *not* fixed: it still fires and degrades individual execs (see the P0.1 row below — the `amd_iommu=off` fix is not staged anywhere, so a reboot would not apply it), but the device keeps producing verified results either side of a burst — on a driver build that predates three BO-teardown/mailbox fixes in that subsystem and comes from the tree upstream deleted in `813e0bf` (detail + evidence in the P0.1 row below, #2459) | `benchmarks/RESULTS-bf16-prefill-CORRECTION-2026-09-12.md` (branch `goal/runlist-decode-wire`) + device check 2026-09-13 |
...
|----|------|:------:|-------|
| P0.1 | NPU exec fault path (IO_PAGE_FAULT per exec, ~10 s/layer) | 🔄 | **The "fix staged" claim that used to sit here is stale — do not reboot expecting it.** Re-checked 2026-09-13: `amd_iommu=off` is **not** in `/boot/grub/grub.cfg` (0 occurrences), not in `/etc/default/grub`, and not in any loader entry, so a reboot returns the same kernel with IOMMU on (`AMD-Vi: Interrupt remapping enabled`, `ivhd0`) and the same faults. The storm is chronic but transient: `aie2_tdr_work: Device isn't making progress` + `IO_PAGE_FAULT` bursts cluster when executions overlap, and the device works either side of them (2026-09-13: a 04:00–05:00 burst of 128 TDR lines, then corr-0.99 bundle checks at 09:31 and successful FLM decodes at 07:04). To actually apply the fix: stage `amd_iommu=off` → `update-grub` → reboot → `research/ws01-npu-attention/validate_npu_after_reboot.sh`. Cheap mitigation meanwhile: **serialize on the device** (rule 4 of the coordination protocol). Original diagnosis 2026-07-31: not a hang — a ~1000×-slow faulting exec; see P01-DIG-FINDINGS.md. **Scope this claim to the driver build (2026-09-17):** the amdxdna behind it is the out-of-tree tree upstream deleted in `813e0bf` — the installed module carries the compile path `src/driver/amdxdna`, taints the kernel as out-of-tree, was built 2026-09-01 11:50, and the checkout under `~/xdna-driver` (`7004f1c`, 2026-08-27) has `ebd297c`, `85f9dcb`, `77e5325` and `813e0bf` all as *non-ancestors* (`git merge-base --is-ancestor`). The one of those checkable by symbol — `xdna_mailbox_drain_channel`, added by `ebd297c` — is absent from both the module and the checkout; a symbol test does not generalise to `77e5325`, which reordered calls to a function that already existed. So "chronic" here means chronic *on a build upstream no longer ships*: `scripts/npu-driver-reapply.sh` prints the installed module's tree (`module_tree_of`) and now refuses to reinstall the deleted one. See #2459. |
