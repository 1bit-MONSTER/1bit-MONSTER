...
- [ ] **COMPLIANCE (HIGH): lemonade re-vendor provenance violates LOCAL-ONLY.** The `third_party/lemonade` re-vendor to `7953d7f6` (HRX backend, ~2026-08-29, commit 43b38b4e) was pulled from **upstream** `github.com/lemonade-sdk/lemonade` — explicitly forbidden by the LOCAL-ONLY rule ("never clone/pull from https://github.com/lemonade-sdk/lemonade"). The local lemonade source (`/home/bcloud/1bit-lemonade-v1170`, branch `chore/lemonade-v11.7.0`) does **NOT** contain commit `7953d7f` (it's a 1bit-MONSTER worktree, and `git cat-file -t 7953d7f` → "Not a valid object name"), so the vendored HRX backend cannot be reproduced from local-only source. `third_party/lemonade/UPSTREAM.md` still instructs `git clone https://github.com/lemonade-sdk/lemonade`, which must not be followed. **Remediation (all local-only, NO upstream contact):** (a) port the HRX backend diff into the local `1bit-lemonade-v1170` worktree and re-vendor from there; or (b) keep the vendored HRX as-is but rewrite `UPSTREAM.md` to source from the local worktree and document that the HRX delta is a local-only patch. Blocked until the local worktree gains the HRX backend or the provenance is corrected. Do NOT re-fetch from upstream.
...
- [ ] P2: `research/ws12-hrx-loom/README.md` stale — its P0/P1 re-vendor, register, smoke-test, and benchmark items are `[ ]` but are actually DONE (tracked as `[x]` in TRACKING.md); reconcile the README to reflect completion.
