...
| 8 | bare-metal linker drops `.bss` statics (#1838) | mlir-aie aiecc ld.script generation | `KERNEL_STATIC` → `.data` placement + build-time `.bss` lint | ⚠️ PARTIAL — see #2199: `KERNEL_STATIC` is present in `engine/npu/kernel/mm_binary_q1.cc` but **absent from the fused kernel** `engine/npu/generators/mm_kernel_reference.cc`, so `g_i4_call` + the local `call` counter sit in `.bss`; `engine/npu/tests/check_kernel_bss.sh` reports `bss=2 / RESULT: FAIL_BSS_ONLY` (measured 2026-09-12). Annotating both moves them to `.data` and the lint then reports `bss=0 / PASS`, but shipping that changes the kernel object, hence the xclbin, and the xclbin cannot be rebuilt here (`xchesscc` fails with `No such device` and the `torch2aie/toolchain` include root is missing `aie_kernel_utils.h`), so the #1897 h2/C2 byte-identity gate cannot be re-established |
