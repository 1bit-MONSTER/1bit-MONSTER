...
| 8 | bare-metal linker drops `.bss` statics (#1838) | mlir-aie aiecc ld.script generation | `KERNEL_STATIC` → `.data` placement + build-time `.bss` lint | ✅ fixed in-tree (issues #1838/#2199): `KERNEL_STATIC` is on the fused kernel's `g_i4_call` + its local `call` counter in `engine/npu/generators/mm_kernel_reference.cc`, as well as in `engine/npu/kernel/mm_binary_q1.cc`. `engine/npu/tests/check_kernel_bss.sh` reports `bss=0 / RESULT: PASS` (re-run 2026-09-12 on `f3825fbb6`), and #2282 shipped the matching xclbin rebuild (67,306 → 75,040 B) with the regenerated provenance manifest and the fused-path `[MoE L1 fused dbg]` corr gate standing in for the #1897 byte-identity check. Caveat unchanged: the xclbin build flow still is not reproducible on this box, so a future `.data` change cannot be gated end-to-end until #2262 lands |
