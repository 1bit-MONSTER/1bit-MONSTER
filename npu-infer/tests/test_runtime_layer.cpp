...
// submission path wired into npu-infer) with the same inputs as the Round-35
// on-device validation, and dump act/logits for byte-comparison.
//
// Build: g++ -O2 -std=c++17 -fpermissive tests/test_runtime_layer.cpp src/runtime_layer.cpp \
//        src/model.c -Iinclude -lxrt_coreutil -lxrt_core -lxrt_core -o /tmp/txn_decode/test_runtime_layer
// Run:   /tmp/txn_decode/test_runtime_layer <model_dir> [dump_prefix]
//        compares against runtime captures when $CAP_DIR is set:
//          act  = $CAP_DIR/actpost_002_*.bin, actpost_004_*.bin
//          logits = /tmp/txn_decode/logits_1000.bin, logits_1001.bin
...
int main(int argc, char** argv) {
    ...
    if (!rt.init(dev, mw, cfg, "captures/txn-elfs",
    ...
    auto cmp_file = [](const char* a, const char* b, const char* what) {
        FILE* fa = fopen(a, "rb"); FILE* fb = fopen(b, "rb");
        if (!fa || !fb) { fprintf(stderr, "  %s: missing file (%s / %s)\n", what, a, b);
            if (fa) fclose(fa); if (fb) fclose(fb); return; }
