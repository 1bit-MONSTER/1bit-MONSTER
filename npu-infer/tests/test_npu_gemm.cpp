...
int main(int argc, char** argv) {
    ...
    // Pack FLOAT32 dequantized weights: the 1MB BO = exactly 256x1024 f32.
    {
        const uint8_t* data = (const uint8_t*)wdata;
        float* f32w = (float*)block;
        for (int r = 0; r < 256; r++) {
            ...
            for (int c = 0; c < 1024; c++) {
                ...
                f32w[r * 1024 + c] = ((float)qv - zp) * scale;
            }
        }
    }
    printf("packed F32 (q-zp)*scale weights (1 MB) to weight BO\n");
