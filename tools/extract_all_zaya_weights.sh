...
set -euo pipefail
cd ${HOME}
WEIGHTS_DIR="${ZAYA_WEIGHTS_DIR:-$HOME/.local/share/1bit-monster/weights}"
mkdir -p "$WEIGHTS_DIR"
...
WEIGHTS_DIR = os.environ.get('ZAYA_WEIGHTS_DIR', os.path.join(os.environ['HOME'], '.local/share/1bit-monster/weights'))
