...
| Gemma3-1B | nh4/nkv1/hd256, qout 1024 | **init FIXED 2026-09-14 — six xclbins were missing, and the "it cannot load" framing was wrong on two counts.** Its shapes (`QKV:1152:1536`, `O:1024:1152`, `G/U:1152:6912`, `D:6912:1152`, fused `GU:1152:13824`) were absent and are now committed; the **column count is constrained** by `(N/n) % n_aie_cols == 0` — at cols=8 every shape core-dumped because `N/128` is 9, 12 or 54 and none is divisible by 8 (`O`/`D` need **cols=3**). And the `k_tile_q4` *"dependency boundary"* is a **WARNING the engine handles**: `[dequant] unaligned dims … tile width taken from the bundle's row size` — then it continues. **Remaining blocker, and it is OURS — root-caused**: the run reaches the layers and dies in **`npu_pack_layer_bo`'s grouped tile reorder**, whose index rule `i = G*(o/G) + (o/2)%S + S*(o%2)` with `S = (G+1)/2` **is a clean permutation for even `G` and is not one for odd `G`** — for `G=9, n=27` it emits `i = 27 = n_tiles` (out of range) and a duplicate, so `src + i*TILE` reads ~35 KB past the tensor. **Gemma3-1B's `G_h = 1152/128 = 9` is the first odd `G` in the set**; the source already warns the rule was *"only ever verified byte-exact for G=8 and G=16 … odd G needs a device run behind it before it is called correct."* **An earlier note in this row blamed the segfault on the missing lm_head ELF — WITHDRAWN; `run_lmhead()` has always had a null check, and the log line was merely the last one printed (a backtrace put the fault in weight packing).** | §860, §865, §870 |
