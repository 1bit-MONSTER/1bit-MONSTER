...
| Qwen3.5-4B | nh16/hd256 | boot 0 | **two reasons, one now concrete**: it is **hybrid** (`GateDeltaNet_prefill.xclbin` + `conv.xclbin` + vision), **and** its I8 rows are in formats the default dequant cannot express — **zero 5120-byte *I8* rows exist in the bundle** (8704 = Q8_0, handled; **4736 = the engine's own MoE trim**, and `rows/20` truncates to 236) — precision matters here: 48 **BF16** tensors do carry `shape[-1] = 2560`, which is **5120 bytes**, because **`shape[-1]` is bytes for I8 and elements for BF16**. An earlier *"row 4736 B is arithmetically malformed"* is **mis-framed**: those bytes are a known packing, not a corrupt file |
