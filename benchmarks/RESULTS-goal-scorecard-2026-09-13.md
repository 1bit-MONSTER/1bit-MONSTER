...
- **Qwen3.5-4B**: three engine-side bugs **found and fixed** on 2026-09-14 (a zero-length BO from an undrived config path; a
  ...
  that one shape in this engine). **All ten gates re-verified after each.** What remains is **a shape-interpretation gap, not
  a format gap**: its STD tensors are **3-D** with a **4736-wide** last dim and the packing reads them as 2-D (§990).
...
| Gemma3-1B | nh4/nkv1/hd256, qout 1024 | **RUNS END TO END as of 2026-09-14 — and its correctness is UNVALIDATED.** The row had been *"fails — dependency boundary, FLM cannot load it either"*; three of its blockers turned out to be engine defects or missing artifacts, not limits. **Six shape xclbins** were absent (now committed; the column count is constrained by `(N/n) % n_aie_cols == 0`, which is why they were never built). The `k_tile_q4` *"dependency boundary"* is a **warning the engine handles**. And two real defects in this engine: the **odd-G tile reorder** (`G_h` = 1152/128 = **9**) and the **byte-extent tile count** (`shape[1]` is BYTES; a row is a tile only at 5120), the second of which also hit `npu_pack_lmhead_bo`. Now: `exit=0`, 26 layers packed, `[1] 0`, **18 ms/tok prefill / 76 tok/s decode** — **but its output is ALL-ZERO LOGITS, and the cause is now established: **the lm_head ELF is absent** (FLM's `gemma_text` class exports no `gen_lm_head_seq`), so the weights pack and **no kernel ever writes the logits** — `[argmax] best=0 (0.00000) … margin=0.00000`. **That artifact is UNOBTAINABLE from either vendor generator**, each refusing with a named error — `gemma_text` exports no `gen_lm_head_seq`, and `qwen3` rejects the config with `Unsupported intermediate size: 6912` — **so the dependency is VENDOR COVERAGE, the same class as Gemma3-4B's `blocks_per_row <= 63`, and FLM cannot load this model either. Recorded as RUNS / OUTPUT EXPLAINED / ARTIFACT UNOBTAINABLE.** The zeros were not an attention, geometry or dequant defect — all three were proposed and eliminated.** Two engine defects were removed (the odd-G reorder and the byte-extent tile count, the latter found in **four** functions by a sweep) | §850, §855, §865-§910 |
| Qwen3.5-4B | nh16/nkv4/hd256, hybrid 3:1 GDN | **MOVED FOUR TIMES on 2026-09-14; the \"boot 0 / format gap\" description was wrong about nearly all of it.** (1) an **engine initialisation-order bug** — xclbin GEMM dims derived on only one config path → **zero-length BO** — **FIXED** (`derive_xclbin_dims()`); (2) a **build gap** — the fused `GU_K2560_N18432` xclbin absent — **FIXED** (four shapes built); (3) **the layer-0 assumption in the STD weight-shape lookups** — Qwen3.5-4B's layer 0 is `linear_attention`, so the `self_attn.*` lookups returned 0 — **FIXED** (first non-GDN layer + both name forms); **all ten gates re-verified after each.** **(4) What remains is a SHAPE-INTERPRETATION gap, not a format gap**: the STD tensors are **3-D** (`q_proj` `[256,10,4736]`, `o_proj` `[80,16,4736]`) with a **4736-wide** last dim, and the STD packing reads them as 2-D `[rows, cols]`, so the count it derives (800, 160) never matches `NH·HD = 4096` and it takes the fused-gate branch. **The dequant has handled 4736-byte rows since §5** — the gap is in the packing's shape reading | §965, §970, §985, §990 |
