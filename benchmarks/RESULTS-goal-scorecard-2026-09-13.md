...
| Qwen3.5-4B | nh16/nkv4/hd256, hybrid 3:1 GDN | **MOVED FIVE TIMES on 2026-09-14; the "boot 0 / format gap" description was wrong about nearly all of it.** (1) an **engine initialisation-order bug** — xclbin GEMM dims derived on only one config path → **zero-length BO** — **FIXED** (`derive_xclbin_dims()`); (2) a **build gap** — the fused `GU_K2560_N18432` xclbin absent — **FIXED**; (3) **the layer-0 assumption in the STD weight-shape lookups** — its layer 0 is `linear_attention` — **FIXED**; (4) **the GDN detection never fired** (a singular `model.layer.` literal against a plural JSON), so **all 32 layers were treated as standard attention** — **FIXED**; (5) **the fused-layout QKV overrun**: that branch packs `q + gate = 2·NH·HD = 8192` rows into a context sized `NH·HD + 2·NKV·HD = 6144` — a **2048-row overrun** — **FIXED** by re-initialising with the **dimension-keyed** xclbin (not the model-tag one, which `xp()` prefers) **and setting `MD/KD/ND` on the members, since `init()` reads them there and not from its args** (credit: the other lane, `6d03d0529`). **All ten gates re-checked green after each.** The remaining gap is the **4736-byte I8 dense rows** — not a shape-reading problem, since `q_cols` has always supplied the 3-D multiply | §965-§1025 |
