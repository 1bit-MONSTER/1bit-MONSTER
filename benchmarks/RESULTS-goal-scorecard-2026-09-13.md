...
| Gemma3-1B | nh4/nkv1/hd256, qout 1024 | **init FIXED 2026-09-14 — six xclbins were missing, and the "it cannot load" framing was wrong on two counts.** Its shapes (`QKV:1152:1536`, `O:1024:1152`, `G/U:1152:6912`, `D:6912:1152`, fused `GU:1152:13824`) were absent and are now committed; the **column count is constrained** by `(N/n) % n_aie_cols == 0` — at cols=8 every shape core-dumped because `N/128` is 9, 12 or 54 and none is divisible by 8 (`O`/`D` need **cols=3**). And the `k_tile_q4` *"dependency boundary"* is a **WARNING the engine handles**: `[dequant] unaligned dims … tile width taken from the bundle's row size` — then it continues. **Remaining blockers are two, and one is OURS**: FLM's `gemma_text` class exports no `gen_lm_head_seq`, so no lm_head ELF can be generated — and **the engine SEGFAULTS on its absence** (`cannot read lm_head ELF` → exit 139) instead of erroring and falling back | §850, §855 |
...
| Gemma3-4B | nh8/nkv4/hd256, qout 2048 | **init FIXED 2026-09-14 — the row was a missing build product, not a limit.** Its five shape xclbins (`QKV:2560:4096`, `O:2048:2560`, `G/U:2560:10240`, `D:10240:2560`) were absent and are now committed; the run proceeds past init. **Next blocker, precisely named**: the runlist needs per-context ELFs and the generator **aborts inside FLM's own code** — `gemma_text_npu_sequence.cpp:92: Assertion 'blocks_per_row <= 63' failed` — so **the vendor's sequence class cannot emit this family's stream**, the same class of limit as Gemma3-1B's hardcoded `k_tile_q4`. **What `blocks_per_row` measures is unresolved**: `intermediate/128` gives 80 for this model, but the families that generate fine give 76 (Qwen3-4B), 112 (Llama-8B) and 64 (Nanbeige) — **all above 63** | §845 |
