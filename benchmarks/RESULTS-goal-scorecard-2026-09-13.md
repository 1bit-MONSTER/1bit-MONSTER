...
| Qwen3.5-4B | nh16/nkv4/hd256, hybrid 3:1 GDN | **MOVED TWICE on 2026-09-14 — the "boot 0 / format gap" description was wrong on both counts.** First it was an **engine initialisation-order bug**: the xclbin GEMM dims were derived only in `parse_q4nx_config`, while the engine's own config path (1BP header, config.json fallback) left them at **zero**, so every `I8Ctx` was a zero-length BO and XRT refused it (`cq before init: MD=128 KD=0`). **Fixed** with a shared `derive_xclbin_dims()` — now `KD=2560 ND=6144`, exactly `pad128(H)`/`pad128(qkv_total)` — and **all ten gates still match**. Second, a **build gap**: the fused `GU_K2560_N18432` was absent; its four shapes are now built and the run **reaches the layer packing** (`per-layer dims detected (all 32 layers)`). **Next site, precisely named**: `layer 3 STD fused: qp=2220545024` → SIGSEGV — layer 3 is the first `full_attention` layer, so the linear layers are handled and the standard-attention branch crashes | §965, §970 |
