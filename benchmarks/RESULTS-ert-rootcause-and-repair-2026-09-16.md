...
## Repair, part 2 (2026-09-16 00:35 ADT): durable + structural

**1. The driver TDR change is now durable.** The live sysfs write is gone after a
module reload or reboot; a config makes it persist:

```
# /etc/modprobe.d/amdxdna-tdr.conf
options amdxdna timeout_in_sec=15
```

Verified: `modprobe -c | grep amdxdna` shows `options amdxdna timeout_in_sec=15`.
Revert: delete the file and `echo 2 | sudo tee /sys/module/amdxdna/parameters/timeout_in_sec`.

**2. The engine now serialises itself — the structural fix.** The TDR raise only
widens the margin; the contention that pushes a submission past it is removed by
making concurrent engine invocations wait instead of queue on the device.

`engine/npu/src/npu_engine_universal.cpp`, at the top of `main()` before any
device work:
    ...
    if (!getenv("NPU_NO_DEVICE_LOCK")) {
        ...
        if (lfd >= 0) {
            if (flock(lfd, LOCK_EX | LOCK_NB) != 0) {
                ...
                if (flock(lfd, LOCK_EX) != 0) ... else fprintf(stderr, "[npu] device lock acquired\n");
            }
        }
    }
```
...
Opt out with `NPU_NO_DEVICE_LOCK=1`; override the path with `NPU_DEVICE_LOCK`.

All **19** `npu_engine_*` binaries were rebuilt (`bash engine/npu/build_npu.sh`,
6m39s CPU-only) and every one contains the lock string.

**Verification** (two invocations, second started 2 s after the first):

```
engine A (background)  -> acquired non-blockingly (silent)
engine B               -> [npu] waiting for the exclusive device lock (/tmp/1bit-npu-device.lock)...
                          [npu] device lock acquired
                       -> === Prefill 40 [runlist] ===  === 97.5 ms/tok (10 tok/s) | tokens=2 ===  rc=0
```

B blocked until A exited, then produced correct runlist output. With the driver
TDR at 15 s and the engine serialised, the contention path that produced
`ERT_CMD_STATE_TIMEOUT` is closed for engine invocations. It does **not** cover
the vendored FastFlowLM binary (which takes no lock) — that remains why the TDR
raise matters as the second layer of the repair.
