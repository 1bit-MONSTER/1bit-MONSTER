...
# /home/bcloud/amd-oss/fastflowlm/src/xclbins/Qwen3-0.6B-NPU2/layer.xclbin -- a tree
# other lanes rebuild. On 2026-09-18 08:19 that file was replaced (401980 B, md5
# fa9f8df2) and every runlist measurement became garbage: the per-ctx ELFs are built
# against the pinned 339980 B / md5 57431faa file, so all 20 oracle rows fell to N
# (0/20) while FLM still scored 18/20. Pin it; set LAYER_XCLBIN to override.
PINNED_LAYER_XCLBIN="$ROOT/engine/npu/xclbins/flm_models/Qwen3-0.6B-NPU2/layer.xclbin"
if [ -z "${LAYER_XCLBIN:-}" ] && [ -f "$PINNED_LAYER_XCLBIN" ]; then
    export LAYER_XCLBIN="$PINNED_LAYER_XCLBIN"
