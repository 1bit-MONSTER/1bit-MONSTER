...
printf 'prompt\texpected\tflm\tnative\tflm_ok\tnative_ok\tnids\tnative_toks\n' > "$OUT"

n=0; flm_ok=0; nat_ok=0; err=0
...
PY
    ...
    nat_n=$(python3 -c "import re;r=open('/tmp/oam_nat.raw',errors='replace').read();print(len(re.findall(r'^\s*\[\d+\]\s+\d+\s*\$',r,re.M))+len(re.findall(r'\[\d+\] batch=\d+ toks:',r)))")
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$prompt" "$expected" "$flm_txt" "$nat_txt" "$fk" "$nk" "$nids" "$nat_n" >> "$OUT"
done < "$SET"
